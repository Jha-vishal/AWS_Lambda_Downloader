...
const lambda = new AWS.Lambda({ region: 'us-east-1' }); // Replace with your region
...
const getLambdaFunctions = async () => {
  ...
  let nextMarker;
  do {
    const { Functions, NextMarker } = await lambda.listFunctions({ Marker: nextMarker }).promise();
    Functions.forEach((f) => {
...
const exportLambda = async (lambdaName) => {
  try {
    const { Configuration } = await lambda.getFunction({ FunctionName: lambdaName }).promise();
    const { Code: { Location } } = await lambda.getFunction({ FunctionName: lambdaName }).promise();
